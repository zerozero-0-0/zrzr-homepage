...
export function getAllArticles() {
  ...
  const files = fs.readdirSync(articlesDir); // ディレクトリ内のファイル一覧取得
  return files
    .filter((f) => f.endsWith(".md")) // .mdファイルのみ対象
    .map((filename) => {
      const filePath = path.join(articlesDir, filename); // 各記事ファイルのパス
      const fileContents = fs.readFileSync(filePath, "utf8"); // ファイル内容を文字列で取得
...
export function getArticleBySlug(slug: string) {
  ...
  const fileContents = fs.readFileSync(filePath, "utf8"); // ファイル内容取得
  const { data, content } = matter(fileContents); // frontmatterと本文を分離
